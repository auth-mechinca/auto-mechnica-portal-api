...
/** The problem this replaced: categories and brands were free text on a part, so
 *  "Electrical" and "Electrical and Charging" became two unrelated values and the
 *  filter list reported both. Nothing could tell a new category from a typo. */
...
describe('over HTTP', () => {
  it('creates on the fly and reports a clash clearly', async () => {
    ...
    assert.match(clash.body.error.message, /Brand "Denso" already exists/);
  });

  it('lists for the dropdown', async () => {
    const res = await request(server).get('/api/ims/brands?q=den').set(auth());
    assert.equal(res.status, 200);
    assert.equal(res.body[0].name, 'Denso');
  });

  it('rejects a blank name', async () => {
    const res = await request(server).post('/api/ims/categories').set(auth()).send({ name: '   ' });
